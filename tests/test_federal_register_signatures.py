...
class GateCorruptionTests(unittest.TestCase):
    ...
    @staticmethod
    def withdraw_existing_signatures(graph):
        """Strip every field `apply_signature_evidence` writes, and nothing else."""
        stack = [graph]
        while stack:
            node = stack.pop()
            stack.extend([c for c in (node.get("children") or []) if isinstance(c, dict)])
            block = node.get("federalRegisterSignature")
            if not isinstance(block, dict):
                continue
            url = str(block.get("url") or "")
            node.pop("federalRegisterSignature", None)
            for key in ("sourceUrls", "evidenceUrls"):
                if isinstance(node.get(key), list):
                    node[key] = [u for u in node[key] if str(u) != url]
            if isinstance(node.get("sourceTypes"), list):
                node["sourceTypes"] = [t for t in node["sourceTypes"] if str(t) != SOURCE_TYPE]
            if str(node.get("verificationMethod") or "") == METHOD:
                node.pop("verificationMethod", None)
            if "sourceCount" in node:
                node["sourceCount"] = len(node.get("sourceUrls") or [])
    ...
    def setUpClass(cls):
        ...
        # The gate is pointed at the WRITTEN fixtures below, so any signature
        # block the published graph already carries cites a document this
        # test's fixture directory does not contain and would be refused as
        # uncommitted. Those blocks are the module's own and are withdrawn
        # here exactly as the exporter's evidence sweep withdraws them, so the
        # record this test writes is the only one of its kind in the graph it
        # validates. Before the first build that published any signature there
        # were none to withdraw, which is why this went unnoticed until three
        # real ones landed.
        cls.withdraw_existing_signatures(graph)
