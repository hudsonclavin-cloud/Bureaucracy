...
DEFAULT_HOUSE = PROJECT_ROOT / "tests" / "fixtures" / "directories" / "house" / "committees.xlsx"
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--house-xlsx", type=Path, default=DEFAULT_HOUSE, help="the House Clerk's verbatim committee spreadsheet (+ .meta.json); pass a missing path to skip")
    ...
    for item in house_report["subcommittees_not_in_list"][:80]:
        print("  not in list:", item)
    for item in house_report["subcommittees_not_in_graph"][:80]:
        print("  not in graph:", item)
    for item in house_report["graph_committees_not_in_list"]:
        print("  committee not in list:", item)
    overlap = set(records) & set(house_records)
    assert not overlap, f"a node in two directories: {sorted(overlap)[:5]}"
    records = {**records, **house_records}
    if args.dry_run:
        return 0
    store = {
        "_note": (
        ...
        "sources": [
            ...
            {"kind": "house_clerk_committee_list", "file": str(args.house_xlsx.relative_to(PROJECT_ROOT)) if args.house_xlsx.is_relative_to(PROJECT_ROOT) else str(args.house_xlsx),
             "url": house[0].get("url") if house else None, "fetched_at": house[0].get("fetched_at") if house else None},
        ],
